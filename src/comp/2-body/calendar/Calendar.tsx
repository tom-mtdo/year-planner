...
export default function Calendar() {
  ...
  const { strYear, numMonth, numDate } = getToday();
  ...
  const months = Array.isArray(calendar) ? (
    calendar.map((aMonth: any, monthIndex: any) => {
      const monthCell = aMonth.map((aDay: DayInfo, dayIndex: any) => {
        ...
        const isToday =
          year === strYear &&
          monthIndex === numMonth && // get month return 0 - 11
          dayIndex === numDate - 1 // get date return 1 - 31
            ? true
            : false;
        return (
      ...
      return (
        <StyledRow key={monthIndex} minHeight={"90px"}>
          ...
          <MonthLabelCell monthIndex={monthIndex} side={'R'}/>
        </StyledRow>
      );
    })
  ) : (
